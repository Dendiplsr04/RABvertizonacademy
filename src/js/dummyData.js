...
export function generateDummyReports() {
  const names = ['Budi Santoso', 'Siti Nurhaliza', 'Ahmad Fauzi', 'Dewi Lestari', 'Rudi Hartono'];
  ...
  for (let i = 0; i < 15; i++) {
    ...
    const report = {
      ...
      category,
      notes: Math.random() > 0.5 ? 'Hasil sangat memuaskan, target tercapai!' : '',
      createdAt: new Date().toISOString(),
    };

    if (category === 'canvasing') {
      report.prospek = Math.floor(Math.random() * 20) + 1;
      report.lokasi = ['Jakarta Selatan', 'Bandung', 'Surabaya', 'Tangerang', 'Bekasi'][Math.floor(Math.random() * 5)];
      ...
      report.platform = platforms[Math.floor(Math.random() * platforms.length)];
      report.viewers = Math.floor(Math.random() * 5000) + 100;
      report.hasil = `Live streaming berjalan lancar dengan engagement rate tinggi.`;
    } else if (category === 'konten') {
      report.jumlahKonten = Math.floor(Math.random() * 10) + 1;
      const selectedJenis = [];
      const count = Math.floor(Math.random() * 3) + 1;
      for (let j = 0; j < count; j++) {
        const jenis = jenisKonten[Math.floor(Math.random() * jenisKonten.length)];
        if (!selectedJenis.includes(jenis)) {
          selectedJenis.push(jenis);
        }
      }
      report.jenisKonten = selectedJenis;
      report.hasil = `Konten berhasil diproduksi dengan kualitas baik dan siap dipublish.`;
    }
